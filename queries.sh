...
echo "$($PSQL "SELECT name FROM teams, games WHERE games.year = 2014 AND games.round = 'Eighth-Final' AND (teams.team_id = games.winner_id OR teams.team_id = games.opponent_id) ORDER BY name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT DISTINCT(name) FROM teams, games WHERE teams.team_id = games.winner_id ORDER BY name")"

echo -e "\nYear and team name of all the champions:"
echo "$($PSQL "SELECT year, name FROM games, teams WHERE games.round = 'Final' AND teams.team_id = games.winner_id ORDER BY year")"
